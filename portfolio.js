    ...
    try {
        ...
        if (!Array.isArray(data.holdings) || !Array.isArray(data.history) ||
            !data.holdings.every(h => h && typeof h.id === "string" && typeof h.name === "string" && h.name.trim() &&
                ...
                Number.isFinite(h.value) && h.value >= 0 && ["style", "cap", "sector"].every(key => portfolioCategories[key].includes(h[key]))) ||
...
function savePortfolio(successMessage) {
    try {
        ...
        portfolioMessage.textContent = "Change applied, but browser storage is unavailable. It will not survive a refresh.";
    }
    renderPortfolio();
}
...
function portfolioTotals() {
    ...
    const total = cash + portfolio.holdings.reduce((sum, h) => sum + h.value, 0);
...
function portfolioAllocation(dimension) {
    ...
    for (const holding of portfolio.holdings) {
        add(dimension === "asset" ? holding.asset : holding.asset === "Stocks" ? holding[dimension] : "Bonds", holding.value);
    }
...
function renderPortfolio() {
    ...
    document.getElementById("portfolio-note").textContent = portfolio.holdings.length
        ? "Based on manually entered holding values. Update them below to reflect the latest portfolio valuation."
    ...
    const rows = [...portfolio.holdings.map(h => [h.name, h.asset, usd(h.value), `${(total ? h.value / total * 100 : 0).toFixed(1)}%`, signed(h.value - h.cost)]),
...
function renderPortfolioControls() {
    ...
    portfolio.holdings.forEach(holding => {
        const item = document.createElement("li");
        const label = document.createElement("span");
        label.textContent = `${holding.name} · ${usd(holding.value)}`;
...
function updateClassificationFields() {
    ["style", "cap", "sector"].forEach(key => holdingForm.elements[key].disabled = holdingForm.elements.asset.value !== "Stocks");
}
